...
async fn main2() -> Result<()> {
    ...
    tokens.push(EthAbiTokenBind::Int {
        data: "1".to_string(),
    });
    tokens.push(EthAbiTokenBind::Uint {
        data: "1".to_string(),
    });
    tokens.push(EthAbiTokenBind::Bool { data: true });
    tokens.push(EthAbiTokenBind::Str {
        data: "test".to_string(),
    });
    tokens.push(EthAbiTokenBind::FixedArray {
        data: vec![datum.clone()],
    });
    tokens.push(EthAbiTokenBind::Array {
        data: vec![datum.clone()],
    });
    tokens.push(EthAbiTokenBind::Tuple {
        data: vec![datum.clone()],
    });
...
async fn main() -> Result<()> {
    println!("test");
    let jsonstring =
        std::fs::read_to_string("../contracts/artifacts/contracts/GameERC721.sol/GameERC721.json")?;
    let json = serde_json::from_str::<serde_json::Value>(&jsonstring)?;
    ...
    let cronosrpc = std::env::var("CRONOSRPC")?;
    println!("contract_address: {}", contract_address);
    println!("cronosrpc: {}", cronosrpc);
    let client: Provider<Http> = Provider::<Http>::try_from(&cronosrpc)?;
    let wallet = fromwallet.clone();
    let wallet = wallet.with_chain_id(338u64);
    let signer: SignerMiddleware<Provider<Http>, Wallet<SigningKey>> =
        SignerMiddleware::new(client, wallet);
    let ethcontract: DynamicContract<SignerMiddleware<Provider<Http>, Wallet<SigningKey>>> =
        DynamicContract::new(&contract_address, &abi_json, signer)?;

    // format string with fromaddress
    let function_args = format!(
        "[{{\"Address\": {{\"data\": \"{:?}\"}}}}, {{\"Str\": {{\"data\": \"myuri\"}}}}]",
        towallet.address()
    );
    // print function_args
    println!("function_args: {}", function_args);
    ...
    let mut ethcontractcall: ContractCall<_, EthDetokenizer> =
        ethcontract.function_call("safeMint", params)?;
    ethcontractcall.contract_call.tx.set_chain_id(338u64);
